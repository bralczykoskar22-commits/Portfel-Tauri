(function () {
  ...
  var APPEARANCE_THEMES = ["aurora", "ocean", "sunset", "forest"];
  var LOGIN_APPEARANCE_KEY = "portfel-login-appearance-v1";
  ...
  function debtScheduleDates(debt, rangeStart, rangeEnd) {
    if (debt.repaymentMode !== "installments" || debt.hasPaymentDay === false) return [];
    var scheduleEnd = debt.hasDeadline ? debt.deadline : "";
    return scheduleDates("monthly", debt.paymentDay, String(debt.createdAt || "").slice(0, 10), scheduleEnd, rangeStart, rangeEnd);
  }
  ...
  function validAppearanceTheme(value) {
    return APPEARANCE_THEMES.indexOf(String(value || "")) >= 0 ? String(value) : "aurora";
  }

  function defaultAppearance() {
    return { loginBackground: "aurora", dashboardBackground: "aurora", motionEnabled: true };
  }

  function readLoginAppearance() {
    var fallback = defaultAppearance();
    try {
      var stored = JSON.parse(window.localStorage.getItem(LOGIN_APPEARANCE_KEY) || "null");
      if (!stored || typeof stored !== "object") return fallback;
      return {
        loginBackground: validAppearanceTheme(stored.loginBackground),
        dashboardBackground: validAppearanceTheme(stored.dashboardBackground),
        motionEnabled: stored.motionEnabled !== false,
  ...
  function saveLoginAppearance(appearance, colorMode) {
    var normalized = {
      loginBackground: validAppearanceTheme(appearance && appearance.loginBackground),
      dashboardBackground: validAppearanceTheme(appearance && appearance.dashboardBackground),
      motionEnabled: !appearance || appearance.motionEnabled !== false,
      colorMode: colorMode === "dark" ? "dark" : "light"
    };
    try { window.localStorage.setItem(LOGIN_APPEARANCE_KEY, JSON.stringify(normalized)); } catch (error) { /* local storage can be unavailable */ }
    return normalized;
  }

  function applyAppearance(appearance, colorMode) {
    var normalized = appearance || defaultAppearance();
    document.documentElement.setAttribute("data-theme", colorMode === "dark" ? "dark" : "light");
    document.documentElement.setAttribute("data-login-background", validAppearanceTheme(normalized.loginBackground));
    document.documentElement.setAttribute("data-dashboard-background", validAppearanceTheme(normalized.dashboardBackground));
    document.documentElement.setAttribute("data-motion", normalized.motionEnabled === false ? "reduced" : "full");
  }

  function applyLoginAppearance() {
    var stored = readLoginAppearance();
    applyAppearance(stored, stored.colorMode);
  }
  ...
  function normalizeData(raw) {
    ...
    var appearanceRaw = settings.appearance && typeof settings.appearance === "object" ? settings.appearance : {};
    var appearance = {
      loginBackground: validAppearanceTheme(appearanceRaw.loginBackground),
      dashboardBackground: validAppearanceTheme(appearanceRaw.dashboardBackground),
      motionEnabled: appearanceRaw.motionEnabled !== false
    };
    ...
    var debts = Array.isArray(raw.debts) ? raw.debts.filter(Boolean).map(function (item) {
      var repaymentMode = item.repaymentMode === "flexible" ? "flexible" : "installments";
      var hasDeadline = item.hasDeadline != null ? item.hasDeadline !== false : !!item.deadline;
      var hasPaymentDay = item.hasPaymentDay != null ? item.hasPaymentDay !== false : repaymentMode === "installments";
      return {
        ...
        deadline: hasDeadline ? String(item.deadline || "") : "",
        ...
        hasPaymentDay: repaymentMode === "installments" && hasPaymentDay,
        paymentDay: Math.min(31, Math.max(1, Math.round(number(item.paymentDay) || 1))),
        createdAt: String(item.createdAt || new Date().toISOString())
      };
    }) : [];
  ...
  function debtPlan(debt, referenceDate) {
    var today = referenceDate || startOfToday();
    var hasDeadline = debt.hasDeadline !== false && !!debt.deadline;
    var deadline = hasDeadline ? dateFromISO(debt.deadline) : null;
    var ledger = debtLedger(debt);
    var remaining = ledger.balance;
    var installmentMode = debt.repaymentMode !== "flexible";
    var hasPaymentDay = installmentMode && debt.hasPaymentDay !== false;
    var dates = deadline && hasPaymentDay ? debtScheduleDates(debt, today, deadline).filter(function (date) {
      var key = isoDate(date);
      return !data.transactions.some(function (transaction) {
    ...
    var minimum = Math.max(0, number(debt.minimumPayment));
    var periods = dates.length;
    if (!periods && deadline && deadline.getTime() >= today.getTime()) periods = monthDistance(today, deadline);
    var required = 0;
    if (remaining > 0 && deadline && periods > 0) {
      required = monthlyRate > 0
        ? remaining * monthlyRate / (1 - Math.pow(1 + monthlyRate, -periods))
        : remaining / periods;
    }
    var recommended = Math.max(required, minimum);
    if (!deadline && minimum <= 0) recommended = 0;
    if (remaining <= 0) recommended = 0;

    var simulatedBalance = remaining;
    var interest = 0;
    var projected = null;
    var cursor = hasPaymentDay
      ? (dates[0] || clampDate(today.getFullYear(), today.getMonth() + 1, debt.paymentDay))
      : addMonths(today, 1);
    if (recommended > simulatedBalance * monthlyRate + .005) {
      for (var index = 0; index < 600 && simulatedBalance > .005; index += 1) {
        ...
        projected = cursor;
        cursor = hasPaymentDay ? clampDate(cursor.getFullYear(), cursor.getMonth() + 1, debt.paymentDay) : addMonths(cursor, 1);
      }
    }
    var deadlinePassed = deadline && deadline.getTime() < today.getTime() && remaining > 0;
    var insufficientPayment = recommended > 0 && monthlyRate > 0 && recommended <= remaining * monthlyRate + .005;
    return {
      remaining: remaining,
      paid: debtPaid(debt),
      dates: dates,
      ...
      impossible: !!deadlinePassed || !!insufficientPayment,
      apr: monthlyRate > 0 ? Math.max(0, number(debt.apr)) : 0,
      installmentMode: installmentMode,
      hasDeadline: hasDeadline,
      hasPaymentDay: hasPaymentDay
    };
  }
  ...
  function renderProfileList() {
    ...
    if (!node) return;
    if (!profiles.length) {
      node.innerHTML = `<div class="empty-plan profile-empty"><strong>Utwórz pierwszy profil</strong><span>Oddziel swoje finanse od innych użytkowników tego urządzenia.</span></div>`;
      return;
    }
    node.innerHTML = profiles.map(function (profile, index) {
      ...
      var security = profile.hasPassword ? "Chroniony hasłem" : "Otwórz jednym kliknięciem";
      var icon = profile.hasPassword
        ? `<svg viewBox="0 0 24 24"><path d="M7 10V8a5 5 0 0 1 10 0v2"></path><path d="M5 10h14v11H5z"></path></svg>`
        : `<svg viewBox="0 0 24 24"><path d="m8 5 7 7-7 7"></path></svg>`;
      return `<button class="profile-card${profile.isDefault ? " is-default" : ""}" style="${avatarStyle}" type="button" data-open-profile="${escapeHTML(profile.id)}">` +
        avatar + `<span class="profile-card-copy"><span class="profile-card-name-row"><strong>${escapeHTML(profile.name)}</strong>${profile.isDefault ? "<em>Domyślny</em>" : ""}</span><small>${security}</small></span>` +
        `<span class="profile-lock" aria-hidden="true">${icon}</span></button>`;
    }).join("");
  }

  function closeProfileLogin() {
    state.selectedLoginProfile = null;
    $("#profile-login-form").hidden = true;
    $("#profile-selection-view").hidden = false;
  ...
  function openProfileLogin(profile) {
    ...
    $("#profile-selection-view").hidden = true;
    $("#profile-login-form").hidden = false;
    $("#profile-login-id").value = profile.id;
    $("#profile-login-name").textContent = profile.name;
    applyProfileAvatar($("#profile-login-avatar"), profile);
    $("#profile-login-password").type = "password";
    $("#profile-login-password").value = "";
    $("#profile-login-error").textContent = "";
    window.setTimeout(function () { $("#profile-login-password").focus(); }, 20);
  }

  async function loginSelectedProfile(profile, password) {
    try {
      $("#profile-gate-error").textContent = "";
      var opened = desktop && desktop.profiles ? await desktop.profiles.login(profile.id, password || "") : profile;
      await activateProfile(opened);
    } catch (error) {
      var message = error && error.message ? error.message : String(error);
      if (profile.hasPassword) $("#profile-login-error").textContent = message;
      else $("#profile-gate-error").textContent = message;
    }
  }

  function submitProfileLogin(event) {
    event.preventDefault();
    var profile = state.selectedLoginProfile || profiles.find(function (item) { return item.id === $("#profile-login-id").value; });
    if (!profile) return;
    loginSelectedProfile(profile, $("#profile-login-password").value);
  }
  ...
  function openProfileDialog() {
    ...
    $("#profile-dialog-id").value = "";
    profileDialogAvatar = "";
    $("#profile-dialog-name-field").hidden = false;
    $("#profile-dialog-color-field").hidden = false;
    $("#profile-dialog-avatar-field").hidden = false;
    $("#profile-dialog-default-field").hidden = false;
    $("#profile-dialog-name").required = true;
    $("#profile-dialog-password-label").textContent = "Hasło opcjonalne";
    $("#profile-dialog-title").textContent = "Dodaj profil";
    $("#profile-dialog-submit").textContent = "Utwórz profil";
    $("#profile-dialog-password").required = false;
    $("#profile-dialog-color").value = "#2563eb";
    openDialog("profile-dialog");
    window.setTimeout(function () { $("#profile-dialog-name").focus(); }, 20);
  }
  ...
  async function submitProfileForm(event) {
    ...
    try {
      ...
      var profile = await desktop.profiles.create({
        name: name,
  ...
  async function loadFromFile() {
    try {
      ...
      dirty = number(raw.version) > 0 && number(raw.version) < 6;
      applyTheme();
      renderAll();
      renderCurrentProfile();
      $("#profile-gate").hidden = true;
  ...
  function applyTheme() {
    ...
    applyAppearance(data.settings.appearance || defaultAppearance(), data.settings.theme);
  }

  function toggleTheme() {
    data.settings.theme = data.settings.theme === "dark" ? "light" : "dark";
    saveLoginAppearance(data.settings.appearance || defaultAppearance(), data.settings.theme);
    applyTheme();
    markDirty();
  }
  ...
  function renderNotificationCenter() {
    ...
    count.textContent = String(currentAlerts.length);
    count.hidden = currentAlerts.length === 0;
    if (!currentAlerts.length) {
      list.innerHTML = '<div class="notification-empty"><span>✓</span><strong>Wszystko pod kontrolą</strong><p>Nie ma teraz spraw wymagających Twojej uwagi.</p></div>';
      return;
    }
    list.innerHTML = currentAlerts.map(function (alert) {
      var tone = alert.priority >= 3 ? "critical" : (alert.priority >= 2 ? "warning" : "info");
      return '<button class="notification-item is-' + tone + '" type="button" data-notification-page="' + alert.page + '">' +
        '<span class="notification-item-dot"></span><span><small>' + escapeHTML(alert.kind) + '</small><strong>' + escapeHTML(alert.name) + '</strong><p>' + escapeHTML(alert.message) + '</p></span><i>→</i></button>';
    }).join("");
  }

  function toggleNotificationDrawer(force) {
    var drawer = $("#notification-drawer");
    var backdrop = $("#notification-backdrop");
    var button = $("#notification-toggle");
    if (!drawer || !backdrop || !button) return;
    var open = typeof force === "boolean" ? force : drawer.hidden;
    drawer.hidden = !open;
    backdrop.hidden = !open;
    button.setAttribute("aria-expanded", String(open));
    document.body.classList.toggle("has-notification-drawer", open);
  }
  ...
  function renderFinancialPulse() {
    var today = startOfToday();
    var month = today.getFullYear() === currentYear() ? today.getMonth() : 0;
    var currentBalance;
    if (modules().bankAccounts) {
      currentBalance = data.accounts.reduce(function (sum, account) { return sum + accountBalance(account, isoDate(today)); }, 0);
    } else {
      var balance = balancesBeforeMonth(month);
      currentBalance = balance.available + monthTransactions(month).reduce(function (sum, transaction) {
        if (String(transaction.date || "") > isoDate(today)) return sum;
        return sum + impact(transaction).available;
      }, 0);
    }
    var stats = monthStats(month);
    var planned = plannedItemsForMonth(month).reduce(function (sum, item) {
      if (item.complete) return sum;
      if (item.kind === "recurring" && item.direction === "income") return sum + number(item.remaining);
      return sum - number(item.remaining);
    }, 0);
    var monthEnd = currentBalance + planned;
    var payday = nextPaydayDate();
    var days = Math.max(0, Math.ceil((payday.getTime() - today.getTime()) / 86400000));
    var obligations = plannedItemsForMonth(month).filter(function (item) {
      return !item.complete && item.date && item.date.getTime() >= today.getTime();
    }).sort(function (a, b) { return a.date - b.date; });
    var next = obligations[0];
    $("#pulse-now").textContent = money(currentBalance);
    $("#pulse-now").classList.toggle("negative", currentBalance < 0);
    $("#pulse-now-note").textContent = stats.income || stats.outflow ? "Po zaksięgowanych operacjach" : "Stan początkowy i konta";
    $("#pulse-month-end").textContent = money(monthEnd);
    $("#pulse-month-end").classList.toggle("negative", monthEnd < 0);
    $("#pulse-month-end-note").textContent = planned >= 0 ? "Plan poprawia saldo o " + money(planned) : "Plan obciąża saldo o " + money(Math.abs(planned));
    $("#pulse-payday").textContent = days === 0 ? "Dzisiaj" : days === 1 ? "1 dzień" : days + " dni";
    $("#pulse-payday-note").textContent = formattedDate(isoDate(payday));
    $("#pulse-next-obligation").textContent = next ? money(next.remaining) : "Brak";
    $("#pulse-next-obligation-note").textContent = next ? next.title + " • " + formattedDate(isoDate(next.date)) : "Brak zaplanowanych płatności";
  }
  ...
  function renderDebts() {
    ...
    $("#debts-summary").innerHTML =
      ...
      `<div class="summary-item"><span>Spłacono</span><strong>${escapeHTML(money(paidTotal))}</strong></div>` +
    ...
    if (!data.debts.length) {
      node.innerHTML = `<div class="empty-targets"><div><strong>Nie masz jeszcze długów</strong><span>Dodaj plan ratalny albo dług spłacany elastycznie. Termin i dzień raty są opcjonalne.</span></div></div>`;
      return;
    }
    node.innerHTML = data.debts.map(function (debt, index) {
      ...
      var info = deadlineInfo(plan.hasDeadline ? debt.deadline : "", plan.remaining <= 0);
      var status = plan.impossible ? `<span class="status-badge is-overdue">Plan wymaga zmiany</span>` : targetStatusHTML(info);
      var next = plan.nextDate ? formattedDate(isoDate(plan.nextDate)) : (plan.hasPaymentDay ? "Brak terminu" : "Bez stałego dnia");
      var projected = plan.projected ? formattedDate(isoDate(plan.projected)) : "Brak danych";
      var modeLabel = plan.installmentMode ? "Raty" : "Bez rat";
      var recommendationLabel = plan.installmentMode ? "Zalecana rata" : "Zalecana wpłata";
      var recommendationCopy = plan.recommended > 0 ? (plan.installmentMode ? "miesięcznie • " + next : "miesięcznie orientacyjnie") : "Ustal własną kwotę spłaty";
      var deadlineTag = plan.hasDeadline ? "do " + formattedDate(debt.deadline) : "bez terminu końcowego";
      var dayTag = plan.hasPaymentDay ? " • dzień raty: " + debt.paymentDay : "";
      return `<article class="target-card is-debt target-card-wide" style="animation-delay:${index * .05}s">` +
        `<div class="target-head"><div><div class="target-title-badges"><span class="target-mode-badge">${modeLabel}</span>${plan.hasDeadline ? '<span class="target-mode-badge is-muted">Termin</span>' : ""}</div><h3>${escapeHTML(debt.name)}</h3><p>Dług: ${escapeHTML(money(debt.total))}</p></div>` +
        ...
        `<div class="recommended-box is-debt"><span>${recommendationLabel}</span><strong>${escapeHTML(money(plan.recommended))}</strong><small>${escapeHTML(recommendationCopy)}</small></div></div>` +
        ...
        `<div class="target-footer">${status}<span class="target-tag">${escapeHTML(deadlineTag + dayTag)}</span></div>` +
        `<div class="target-actions"><button class="button button-primary compact-button" type="button" data-pay-debt="${escapeHTML(debt.id)}">Dodaj spłatę</button>` +
  ...
  function renderFinancialInsights(rows) {
    var node = $("#financial-insights");
    if (!node) return;
    var insights = [];
    var month = startOfToday().getFullYear() === currentYear() ? startOfToday().getMonth() : 0;
    var current = monthStats(month);
    var previous = month > 0 ? monthStats(month - 1) : null;
    if (previous && previous.expense > 0) {
      var change = (current.expense - previous.expense) / previous.expense * 100;
      insights.push({ tone: change > 5 ? "warning" : "positive", title: "Wydatki miesiąca", copy: change > 0 ? "Są o " + Math.abs(Math.round(change)) + "% wyższe niż miesiąc wcześniej." : "Są o " + Math.abs(Math.round(change)) + "% niższe niż miesiąc wcześniej." });
    }
    var lowest = rows.reduce(function (selected, row) { return row.balance < selected.balance ? row : selected; }, rows[0]);
    insights.push({ tone: lowest.balance < 0 ? "danger" : "neutral", title: "Najniższe prognozowane saldo", copy: MONTHS[lowest.month] + ": " + money(lowest.balance) + "." });
    var overdue = collectNotifications().filter(function (item) { return item.tone === "danger"; }).length;
    insights.push({ tone: overdue ? "danger" : "positive", title: "Terminy i zaległości", copy: overdue ? overdue + " spraw wymaga pilnej reakcji." : "Nie ma obecnie pilnych zaległości." });
    var goalBehind = data.goals.filter(function (goal) { return goalPlan(goal).paceClass === "is-behind"; }).length;
    if (data.goals.length) insights.push({ tone: goalBehind ? "warning" : "positive", title: "Cele oszczędnościowe", copy: goalBehind ? goalBehind + " cel(e) są poniżej planowanego tempa." : "Wszystkie aktywne cele są zgodne z planem." });
    node.innerHTML = insights.map(function (item) { return '<article class="financial-insight is-' + item.tone + '"><i></i><div><strong>' + escapeHTML(item.title) + '</strong><p>' + escapeHTML(item.copy) + '</p></div></article>'; }).join("");
  }
  ...
  function setSettingsTab(tab) {
    var valid = ["profile", "appearance", "finance", "modules", "data", "app"];
    state.settingsTab = valid.indexOf(tab) >= 0 ? tab : "profile";
    $$("[data-settings-tab]").forEach(function (button) { button.classList.toggle("is-active", button.getAttribute("data-settings-tab") === state.settingsTab); });
    $$("[data-settings-panel]").forEach(function (panel) {
      var active = panel.getAttribute("data-settings-panel") === state.settingsTab;
      panel.hidden = !active;
      panel.classList.toggle("is-active", active);
    });
  }

  function selectedTheme(name, fallback) {
    var checked = $('input[name="' + name + '"]:checked');
    return validAppearanceTheme(checked ? checked.value : fallback);
  }

  function previewAppearanceSettings() {
    if (!data) return;
    var next = {
      loginBackground: selectedTheme("login-theme", data.settings.appearance.loginBackground),
      dashboardBackground: selectedTheme("dashboard-theme", data.settings.appearance.dashboardBackground),
      motionEnabled: $("#settings-motion-enabled").checked
    };
    applyAppearance(next, $("#settings-color-mode").value);
  }

  function submitAppearanceSettings(event) {
    event.preventDefault();
    data.settings.theme = $("#settings-color-mode").value === "dark" ? "dark" : "light";
    data.settings.appearance = {
      loginBackground: selectedTheme("login-theme", "aurora"),
      dashboardBackground: selectedTheme("dashboard-theme", "aurora"),
      motionEnabled: $("#settings-motion-enabled").checked
    };
    saveLoginAppearance(data.settings.appearance, data.settings.theme);
    applyTheme();
    markDirty("Zapisano wygląd programu.");
  }

  function openGateAppearanceDialog() {
    var stored = readLoginAppearance();
    $$('input[name="gate-login-theme"]').forEach(function (input) { input.checked = input.value === stored.loginBackground; });
    $("#gate-motion-enabled").checked = stored.motionEnabled !== false;
    openDialog("appearance-dialog");
  }

  function submitGateAppearance(event) {
    event.preventDefault();
    var stored = readLoginAppearance();
    var appearance = {
      loginBackground: selectedTheme("gate-login-theme", stored.loginBackground),
      dashboardBackground: stored.dashboardBackground,
      motionEnabled: $("#gate-motion-enabled").checked
    };
    saveLoginAppearance(appearance, stored.colorMode);
    applyLoginAppearance();
    closeDialog("appearance-dialog");
  }
  ...
  function renderSettings() {
    ...
    var appearance = data.settings.appearance || defaultAppearance();
    $("#settings-color-mode").value = data.settings.theme === "dark" ? "dark" : "light";
    $("#settings-motion-enabled").checked = appearance.motionEnabled !== false;
    $$('input[name="login-theme"]').forEach(function (input) { input.checked = input.value === appearance.loginBackground; });
    $$('input[name="dashboard-theme"]').forEach(function (input) { input.checked = input.value === appearance.dashboardBackground; });
  ...
  function fillGoalPaydayOptions() {
    ...
    $("#goal-payday-recurring").innerHTML = incomes.length ? incomes.map(function (item) {
      return "<option value=\"" + escapeHTML(item.id) + "\">" + escapeHTML(item.name + " • " + money(item.expectedAmount)) + "</option>";
  ...
  function openDebtDialog(id) {
    ...
    if (debt) {
      ...
      $("#debt-repayment-mode").value = debt.repaymentMode === "flexible" ? "flexible" : "installments";
      $("#debt-has-deadline").checked = debt.hasDeadline !== false && !!debt.deadline;
      ...
      $("#debt-has-payment-day").checked = debt.hasPaymentDay !== false;
      $("#debt-payment-day").value = number(debt.paymentDay) || 1;
      $("#debt-dialog-title").textContent = "Edytuj dług";
    } else {
      $("#debt-repayment-mode").value = "installments";
      $("#debt-has-deadline").checked = true;
      $("#debt-apr").value = 0;
      $("#debt-interest-enabled").checked = false;
      $("#debt-minimum-payment").value = 0;
      $("#debt-has-payment-day").checked = true;
  ...
  function updateDebtFields() {
    var installments = $("#debt-repayment-mode").value === "installments";
    var hasDeadline = $("#debt-has-deadline").checked;
    var hasPaymentDay = installments && $("#debt-has-payment-day").checked;
    $("#debt-deadline-field").hidden = !hasDeadline;
    $("#debt-deadline").required = hasDeadline;
    $("#debt-installment-fields").classList.toggle("is-flexible", !installments);
    $("#debt-has-payment-day").closest("label").hidden = !installments;
    $("#debt-payment-day-field").hidden = !hasPaymentDay;
    $("#debt-payment-day").required = hasPaymentDay;
    $("#debt-payment-amount-label").textContent = installments ? "Kwota raty" : "Planowana miesięczna wpłata";
    $("#debt-mode-note-title").textContent = installments ? "Plan ratalny" : "Elastyczna spłata";
    $("#debt-mode-note-copy").textContent = installments
      ? (hasPaymentDay ? "Raty pojawią się w kalendarzu w wybranym dniu miesiąca." : "Raty nie mają stałego dnia — program pokaże jedynie zalecaną kwotę.")
      : (hasDeadline ? "Wpłacasz dowolne kwoty, a program oblicza tempo potrzebne do terminu." : "Wpłacasz dowolne kwoty bez narzuconego harmonogramu i terminu.");
  ...
  function submitDebt(event) {
    ...
    var repaymentMode = $("#debt-repayment-mode").value === "flexible" ? "flexible" : "installments";
    var hasDeadline = $("#debt-has-deadline").checked;
    var deadline = hasDeadline ? $("#debt-deadline").value : "";
    ...
    var hasPaymentDay = repaymentMode === "installments" && $("#debt-has-payment-day").checked;
    var paymentDay = Math.round(number($("#debt-payment-day").value) || 1);
    var error = $("#debt-error");
    error.textContent = "";
    if (!name || !(total > 0)) {
      error.textContent = "Podaj nazwę i kwotę długu.";
      return;
    }
    if (hasDeadline && !deadline) {
      error.textContent = "Podaj termin spłaty albo wyłącz konkretny termin.";
      return;
    }
    if (hasDeadline && dateFromISO(deadline).getTime() < startOfToday().getTime() && !(existing && debtRemaining(existing) <= 0)) {
      error.textContent = "Termin spłaty nie może być w przeszłości.";
      return;
    }
    if (apr < 0 || minimumPayment < 0 || (hasPaymentDay && (paymentDay < 1 || paymentDay > 31))) {
      error.textContent = "Sprawdź oprocentowanie, kwotę wpłaty i dzień raty.";
      return;
    }
  ...
  function handleDocumentClick(event) {
    ...
    if (openProfile) {
      ...
      if (profile && profile.hasPassword) openProfileLogin(profile);
      else if (profile) loginSelectedProfile(profile, "");
      return;
    }
    var settingsTab = event.target.closest("[data-settings-tab]");
    if (settingsTab) {
      setSettingsTab(settingsTab.getAttribute("data-settings-tab"));
      return;
    }
  ...
  function bindEvents() {
    ...
    $("#profile-gate-settings").addEventListener("click", openGateAppearanceDialog);
    $("#gate-appearance-form").addEventListener("submit", submitGateAppearance);
    $("#profile-login-form").addEventListener("submit", submitProfileLogin);
    $("#profile-login-back").addEventListener("click", closeProfileLogin);
    $("#profile-password-toggle").addEventListener("click", function () {
      var input = $("#profile-login-password");
      input.type = input.type === "password" ? "text" : "password";
      this.setAttribute("aria-label", input.type === "password" ? "Pokaż hasło" : "Ukryj hasło");
    });
    ...
    $("#notification-toggle").addEventListener("click", function () { toggleNotificationDrawer(); });
    $("#notification-close").addEventListener("click", function () { toggleNotificationDrawer(false); });
    $("#notification-backdrop").addEventListener("click", function () { toggleNotificationDrawer(false); });
    $("#notification-list").addEventListener("click", function (event) {
      var button = event.target.closest("[data-notification-page]");
      if (!button) return;
      toggleNotificationDrawer(false);
      setPage(button.getAttribute("data-notification-page"));
    });
    ...
    $("#debt-repayment-mode").addEventListener("change", updateDebtFields);
    $("#debt-has-deadline").addEventListener("change", updateDebtFields);
    $("#debt-has-payment-day").addEventListener("change", updateDebtFields);
    ...
    $("#appearance-settings-form").addEventListener("submit", submitAppearanceSettings);
    $("#settings-color-mode").addEventListener("change", previewAppearanceSettings);
    $("#settings-motion-enabled").addEventListener("change", previewAppearanceSettings);
    $$('input[name="login-theme"], input[name="dashboard-theme"]').forEach(function (input) { input.addEventListener("change", previewAppearanceSettings); });
