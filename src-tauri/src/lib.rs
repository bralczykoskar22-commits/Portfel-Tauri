...
fn empty_data() -> Value {
    ...
    json!({
        ...
        "settings": {
            ...
            "theme": "light",
            "appearance": {
                "loginBackground": "aurora",
                "dashboardBackground": "aurora",
                "motionEnabled": true
            },
            "currency": "PLN",
...
fn migrate_payload(mut payload: Value) -> Result<Value, String> {
    ...
    let opening_available = {
        ...
        for (key, default_value) in [
            ...
            ("appearance", &defaults_settings["appearance"]),
        ] {
            let target = settings
                .entry(key.to_string())
                .or_insert_with(|| Value::Object(Map::new()));
        ...
        let appearance = settings
            ...
            .expect("wygląd został ustawiony jako obiekt");
        for key in ["loginBackground", "dashboardBackground"] {
            let theme = match appearance.get(key).and_then(Value::as_str) {
                Some("ocean") => "ocean",
                Some("sunset") => "sunset",
                Some("forest") => "forest",
                _ => "aurora",
            };
            appearance.insert(key.to_string(), json!(theme));
        }
        let motion_enabled = appearance
            .get("motionEnabled")
            .and_then(Value::as_bool)
            .unwrap_or(true);
        appearance.insert("motionEnabled".to_string(), json!(motion_enabled));
    ...
    if let Some(debts) = object.get_mut("debts").and_then(Value::as_array_mut) {
        for item in debts {
            let Some(debt) = item.as_object_mut() else {
                continue;
            };
            let repayment_mode = if debt
                .get("repaymentMode")
            ...
            let has_payment_day = repayment_mode == "installments"
                && debt
                    .get("hasPaymentDay")
                    .and_then(Value::as_bool)
                    .unwrap_or(true);
            let payment_day = number_value(debt.get("paymentDay")).round().clamp(1.0, 31.0) as i64;

            debt.insert("repaymentMode".to_string(), json!(repayment_mode));
            debt.insert("hasDeadline".to_string(), json!(has_deadline));
            debt.insert("hasPaymentDay".to_string(), json!(has_payment_day));
            debt.insert("paymentDay".to_string(), json!(payment_day));
            if !has_deadline {
                debt.insert("deadline".to_string(), json!(""));
            }
        }
    }
...
fn open_database(paths: &StoragePaths) -> Result<Connection, String> {
    ...
    if !has_state {
        ...
        connection
            .execute(
                "INSERT INTO app_state(id, schema_version, payload, updated_at) VALUES(1, 6, ?1, ?2)",
    ...
    if !has_profiles {
        ...
        connection
            .execute(
                "INSERT INTO profile_state(profile_id, schema_version, payload, updated_at) VALUES('profile-default', 6, ?1, ?2)",
...
fn create_profile(
    ...
    transaction
        .execute(
            "INSERT INTO profile_state(profile_id, schema_version, payload, updated_at) VALUES(?1, 6, ?2, ?3)",
...
mod tests {
    ...
    #[test]
    fn migrates_old_cash_data_to_version_six() {
        let old = json!({
            "version": 3,
            "settings": {
                "currentYear": 2026,
